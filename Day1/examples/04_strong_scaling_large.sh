...
#SBATCH --time=0:59:00               		# Max wall time
#SBATCH --qos=normal                 		# Specify debug QOS
#SBATCH --partition=shas             		# Specify Summit haswell nodes
#SBATCH --ntasks=24          	 	        # Number of tasks per job
#SBATCH --job-name=large_strong_pi_bench              # Job submission name
#SBATCH --output=large_strong_pi_bench.%a.%j.out      # Output file name with Job ID
...
# output file name
printf -v OUTFILE "large_strong_pi.%02d.%s.out" $SLURM_ARRAY_TASK_ID $SLURM_JOBID

# Run python code - large_strong scaling: problem size stays constant
