...
#SBATCH --job-name=weak_pi_bench                # Job submission name
#SBATCH --output=weak_pi_bench.%a.%j.out        # Output file name with Job ID
...
# output file name
printf -v OUTFILE "weak_pi.%02d.%s.out" $SLURM_ARRAY_TASK_ID $SLURM_JOBID

# Run python code - weak scaling: problem size increases with number of cores

problem_size=$(( SLURM_ARRAY_TASK_ID*10000000 ))
